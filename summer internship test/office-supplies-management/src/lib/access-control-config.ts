...
export const ROLE_ACCESS_CONFIG: Record<UserRole, AccessControlConfig> = {
  ...
  MANAGER: {
    ...
    users: {
      canView: true, // Can view users from their department for requester dropdown
      canCreate: false,
      canEdit: false,
      canDelete: false,
    ...
    departments: {
      canView: true, // Can view departments for request creation
      canCreate: false,
      canEdit: false,
      canDelete: false,
 ...
 */
export function getAdditionalRestrictions(role: UserRole, feature: keyof AccessConfig): string[] {
  const config = getAccessConfig(role)
  const featureConfig = config[feature] as FeatureAccess
