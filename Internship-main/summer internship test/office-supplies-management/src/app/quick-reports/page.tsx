...
import { BarChart3, TrendingUp, AlertTriangle, Package, DollarSign, Calendar, Download, RefreshCw, Filter } from 'lucide-react'
...
export default function QuickReportsPage() {
  ...
  const getExportData = () => {
    ...
    switch (activeTab) {
      case 'consumption':
        return {
          departments: reportData.topConsumingDepartments.map(dept => ({
            'Department': dept.department,
            'Total Consumed': dept.totalConsumed,
            'Total Cost': dept.totalCost
          })),
          items: reportData.mostConsumedItems.map(item => ({
      ...
      case 'cost':
        return {
          categories: reportData.costByCategory.map(cat => ({
            'Category': cat.category,
            'Total Cost': cat.totalCost,
            'Item Count': cat.itemCount
          })),
          departments: reportData.costByDepartment.map(dept => ({
            'Department': dept.department,
            'Total Cost': dept.totalCost,
      ...
      case 'forecast':
        return {
          predictions: reportData.predictedDemand.map(pred => ({
          ...
          lowStock: reportData.lowStockItems.map(item => ({
            'Item Name': item.itemName,
